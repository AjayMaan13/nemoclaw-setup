...
The `.env` file is not loaded automatically — you must export the key into your shell first, then run the wizard:

```bash
export NVIDIA_API_KEY=$(grep NVIDIA_API_KEY workshop/.env | cut -d= -f2) && nemoclaw onboard
```

> **Why the export?** `.env` files are just text — the shell doesn't read them unless told to. The `export` command loads the key into your shell session so the wizard can find it.
...
When asked for a provider, select **1 (NVIDIA Endpoints)**.
...
Turn your agent into a Telegram bot in 3 steps.
...
Open `workshop/.env` and uncomment the Telegram line:
...
```bash
export TELEGRAM_BOT_TOKEN=$(grep TELEGRAM_BOT_TOKEN workshop/.env | cut -d= -f2)
nemoclaw stop
nemoclaw start
```

### Step 3 — Apply the Telegram security policy

NemoClaw blocks all outbound traffic by default. Unlock Telegram for your sandbox:

```bash
nemoclaw <your-agent-name> policy-add
```

Select **telegram** from the list. This allows your agent to reach `api.telegram.org`.

Open your bot in Telegram and send it a message — your agent will reply.
